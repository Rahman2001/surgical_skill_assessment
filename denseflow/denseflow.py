...
import imageio
skvideo.setFFmpegPath("C:/ffmpeg-master-latest-win64-gpl/bin/")
...
def save_flows(data_root, new_dir, flows, image, save_dir, num, bound):
    """
    ...
    flow_x_img = flow_x_img.convert("L")
    flow_y_img = Image.fromarray(flow_y)
    flow_y_img = flow_y_img.convert("L")
...
def dense_flow(augs):
    ...
    :param augs:the detailed augments:
        videos_root: the absolute parent source to videos folder
    ...
    :return: no returns
    """
    video_root, video_name, save_dir, step, bound, new_dir = augs
    ...
    #     exit()
    try:
        print(f'This is video_path: {video_path}')
    ...
    while True:
        ...
        save_flows(video_root, new_dir, flowDTVL1, image, save_dir, frame_num, bound)  # this is to save flows and img.
        prev_gray = gray
        prev_image = image
        frame_num += 1
        # to pass the out of stepped frames
...
if __name__ == '__main__':
    ...
    print(f'This is data_root = {data_root}')
    videos_root = os.path.join(data_root, 'video')
    print(f'This is videos_root = {videos_root}')
    ...
    if mode == 'run':
        # pool.map(dense_flow, zip([videos_root] * len(video_list), video_list, flows_dirs,
                                # [step] * len(video_list), [bound] * len(video_list), [new_dir] * len(video_list)))
        for i in range(len(video_list)):
            dense_flow((videos_root, video_list[i], flows_dirs[i], step, bound, new_dir))
    else:  # mode=='debug
        dense_flow((videos_root, video_list[0], flows_dirs[0], step, bound, new_dir))
